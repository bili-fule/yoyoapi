<script lang="ts">
  import { sendCode, resetPassword } from '$lib/api/index.js'
  import { goto } from '$app/navigation'

  let step = $state<'code' | 'reset'>('code')
  let email = $state('')
  let code = $state('')
  let password = $state('')
  let error = $state('')
...
<svelte:head>
  <title>Reset Password - YoYOapi</title>
</svelte:head>
  ...
  {:else}
    <form onsubmit={handleReset}>
      {#if error}
        <div class="error">{error}</div>
      {/if}
      <p class="info">Reset code sent to {email}</p>
      <label>
        Verification Code
